...
const staticObj = {
	title: `${new Date().getFullYear()} - ${new Date().getFullYear() + 1} MBBS ADMISSION IN ABROAD`,
...
const faq = [
	...
	},
	{
		...
		desc: `<p>We are keeping a close watch on COVID-19 effects on MBBS Abroad education. As per the current situation, countries that are famous for MBBS Abroad such as Ukraine, Russia, and Philippines have minor effects and have contained the virus very well. According to the reviews from countries and universities, they will be accepting the admissions in MBBS in ${new Date().getFullYear()} - ${new Date().getFullYear() + 1} without any restrictions. MBBS abroad ${new Date().getFullYear()} - ${new Date().getFullYear() + 1} students will be traveling in October ${new Date().getFullYear()} - ${new Date().getFullYear() + 1} and by that time the complete situation will be under control. So, the students no need to worry, admissions in MBBS Abroad ${new Date().getFullYear()} - ${new Date().getFullYear() + 1} will remain unaffected and New-Lyf MBBS Abroad Consultancy in India will help you get admission in MBBS abroad smoothly and safely.</p>`,
	},
];
				...
				<Grid item xs={11}>
					<Grid container spacing={2}>
						...
						<Grid item xs={12} md={6}>
							<img src={"/assests/home-page-banner-1.png"} width="100%" />
						</Grid>
						<Grid item xs={12} md={5.5} sx={{display: "flex", alignItems: "center"}}>
							<Box>
								<Typography variant="h3" className={styles.subTitle} gutterBottom>
									"Don't Worry We Are Here To Assist You"
								</Typography>
								<Typography variant="body1" gutterBottom>
									Grab the top abroad education consultant to fulfill your aspiration with:
								</Typography>
								{studyAbroadList.map((item, index) => (
									<Box sx={{display: "flex", alignItems: "center"}} key={index}>
										<Typography className={styles.circleNum} sx={{color: "primary.main"}} variant="body1">
											{index + 1}
										</Typography>
