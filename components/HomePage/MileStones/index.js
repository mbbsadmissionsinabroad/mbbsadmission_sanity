...
const data = [
	...
	},
	{
		icon: "AccountSchool",
		title: "3000+",
		desc: "tudents Already Enrolled Through Us"
	},
	{
		icon: "Handshake",
		title: "250+",
						...
						data.map((item, i) => {
							...
							return(
								<Grid item xs={12} md={3} sx={{ textAlign: 'center' }} key={i}>
									...
									<Typography variant="h3" sx={{ pb: 1.5 }} className={styles.mileStonesTitle}>{item.title}</Typography>
									<Typography variant="body1" sx={{ color: "#fff" }}>{item.desc}</Typography>
								</Grid>
							)
						})
					}
