...
function Footer({footerData}) {
	...
	return (
		<>
			<Box className="footer-container" style={{display:'flex'}}>
				<div>
				<Typography variant="body1" color="white" gutterBottom sx={{ textAlign: 'center', fontSize: '20px' }}>Quick Links</Typography>
				<ul className="footer-li">
					{
						navItems.length > 0 && (
							navItems.map((item, i) => <li key={i} style={{ listStyleType: 'none', padding:"4px" }}><Link href={item.slug}><a style={{ color: "#fff", display: 'flex', alignItems: 'center' }}><Stethoscope sx={{ fontSize: '16px', mr: 2}} /> {item.title}</a></Link></li>)
						)
					}
				</ul>
				</div>
				...
				<Box >
				<Link href="/blog">
					<button className="button-footer" text="BLOG">BLOG</button>
						{/* <Typography variant="body1">Blog</Typography> */}
					{/* </a> */}
				</Link>
				<Link href="/gallery">
					<a>
					<button className="button-footer" sx={{ textAlign: 'center', fontSize: '20px' }} text="GALLERY">GALLERY</button>
					</a>
				</Link>
				...
				<Box display="flex" alignItems="center" justifyContent="center" sx={{ mt: 2.5, "a": { color: '#fff !important' } }}>
					...
					<a href="https://www.youtube.com/channel/UCGdRZ74SghrH9K8BE5U0zVw" target="_blank"><Youtube className="social-icon" /></a>
				</Box>
				</div>
